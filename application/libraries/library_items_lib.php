	...
	function return_rendered_items($library_id,$replace_with = '',$replace_with_array = '',$value_id = 0)
	{
		...
		if(is_array($this->list) and count($this->list)>0)
		{
		    foreach($this->list as $k => $row)
		    {
		    	...
		    	{
		    		case "T":
		    			$item_to_render = '<input class="form-control" type="text" id="item_'.$row['id'].'" name="items[replace]['.$row['id'].']" value="[value]" placeholder="'.$row['help'].'" />';
		    		break;
		    		case "TA":
		    			$item_to_render = '<textarea id="item_'.$row['id'].'" name="items[replace]['.$row['id'].']" class="form-control autogrow" placeholder="'.$row['help'].'">[value]</textarea>';
		    		break;
		    		case "DATE":
		    			$item_to_render = '<input id="item_'.$row['id'].'" name="items[replace]['.$row['id'].']" class="form-control col-md-2 datepicker" size="16" type="text" value="[value]">';
		    		break;
		    		case "TIME":
		    			$item_to_render = '<input id="item_'.$row['id'].'" name="items[replace]['.$row['id'].']" class="form-control col-md-2 timepicker" size="16" type="text" value="[value]">';
		    		break;
		    		case "R":
		    		...
		    		break;
		    		case "D":
		    			$item_to_render = '<select id="item_'.$row['id'].'" name="items[replace]['.$row['id'].']" data-placeholder="'.$row['help'].'" class="form-control chosen-select">';
		    		...
		    		case "LIBRARY":
		    			...
		    			//die($row['var_value']);
		    			$temp = explode(OPT_SEPERATOR,$row['var_value']);
		    		...
		    		break;
		    		case "LIBRARY_MSEL":
		    			$item_to_render = '<select id="item_'.$row['id'].'" name="items[replace]['.$row['id'].'][]" data-placeholder="'.$row['help'].'" class="form-control chosen-select" multiple>';
		    			$item_to_render .= '<option value=""></option>';
		    		...
		    		break;
		    		case "MSEL":
		    			$item_to_render = '<select id="item_'.$row['id'].'" name="items[replace]['.$row['id'].'][]" class="form-control chosen-select" multiple>';
	    				$res = $this->ci->db->query("select dropdown_id,dropdown_item_id,`name` from `dropdown_item` where is_active='Yes' and dropdown_id=".$row['var_value']);
	    				if($res->num_rows()>0)
	    				{
	    					foreach ($res->result_array() as $data)
		    			...
		    			if($value_id)
		    			{
			    			...
			    			<div id="task_list">
			    				<div id="task_template" class="row hidden" style="margin-bottom:5px">
			    					<div class="col-md-12">
				    					<div class="pull-left text-right" style="width:30px;padding-top:10px">
				    					...
				    					<div class="pull-left" style="width:350px;margin-left:10px">
				    						...
				    						<input type="hidden" class="task_priority" name="" value="0" />
				    					</div>
			    			...
			    			<script>
			    				...
			    				function new_task()
			    				{
			    					...
			    					$template.find(".task_priority").attr("name",current_task_num);
			    				...
			    				function show_task(task_id,title,assigned_to,task_status,task_priority)
			    				{
			    					task_priority = typeof task_priority == "undefined"?0:task_priority;
			    					
			    					var $template = $("#task_template").clone();
			    					$template.removeClass("hidden").find("select").addClass("chosen-select");
			    					$template.find(".num").html(current_task_num);
			    					$template.find(".task_id").val(task_id).attr("name","tasks["+task_id+"][task_id]");
			    					$template.find(".task_priority").val(task_priority).attr("name","tasks["+task_id+"][task_priority]");
			    				...
			    				if(tasks.length>0)
			    				{
			    					for(x in tasks)
			    					{
			    						...
			    						show_task(task["task_id"],task["title"],task["assigned_to"],task_status,task["priority"]);
			    					}
			    				}
			    			</script>';
		    		...
		    		case "TEAM":
		    			...
		    			//if($value_id)
		    			//{
		    				$this->ci->load->library("teams_lib");
		    				$this->ci->load->library("library_values");
		    				
		    				$employees = $this->ci->library_values->select_library_columns_with_data(0,'Employees',array("First Name","Last Name"));
		    				
		    				$user_options = '';
		    				if(is_array($employees) and count($employees)>0)
		    				{
		    					foreach ($employees as $ek => $ev) {
		    						$user_options .= '<option value="'.$ev['First Name'].' '.$ev['Last Name'].'">'.$ev['First Name'].' '.$ev['Last Name'].'</option>';
		    					}
		    				}
		    				
		    				$teams = $this->ci->teams_lib->get_by_library_value($value_id);
		    				
		    				$item_to_render = '
		    				<style>
		    					.team_row{ padding:2px; }
		    				</style>
		    				<div id="team_list">
		    					<div id="team_template" class="row hidden" style="margin-bottom:5px">
		    						<div class="col-md-12">
		    							<div class="pull-left text-right" style="width:30px;padding-top:10px">
		    								<span style="cursor: move;" class="icon icon-th-list handle"></span>&nbsp;<span class="num">1</span>
		    							</div>
		    							<div class="pull-left" style="width:180px;padding-top:5px;margin-left:10px">
		    								<select name="" class="form-control team_member">
		    									'.$user_options.'
		    								</select>
		    							</div>
		    							<div class="pull-left" style="width:180px;margin-left:10px">
		    								<input type="text" class="form-control team_role" name="" value="" placeHolder="Role" />
		    							</div>
		    							<div class="pull-left" style="width:350px;margin-left:10px">
		    								<input type="text" class="form-control team_responsibility" name="" placeholder="Responsibility Here" />
		    								<input type="hidden" class="team_id" name="" value="0" />
		    								<input type="hidden" class="team_priority" name="" value="0" />
		    							</div>
		    							<div class="pull-left" style="width:10px; padding-top:10px;margin-left:10px">
		    								<a href="javascript:;" class="btn_delete" onclick="delete_this_team_member($(this))"><span class="icon icon-remove"></span></a>
		    							</div>
		    				...
		    				<div class="padd2">
		    					<a href="javascript:;" onclick="new_team_member()" class="btn btn-xs btn-success">Add User</a>
		    				</div>
		    				<script>
		    					var current_team_num=1;
		    					var teams = '.json_encode($teams).';
		    					function new_team_member()
		    					{
		    						var $template = $("#team_template").clone();
		    						$template.removeClass("hidden").find("select").addClass("chosen-select");
		    						$template.find(".num").html(current_team_num);
		    						$template.find(".team_priority").attr("name","teams[N"+current_team_num+"][team_priority]").val(current_team_num);
		    						$template.find(".team_id").attr("name","teams[N"+current_team_num+"][team_id]");
		    						$template.find(".team_member").attr("name","teams[N"+current_team_num+"][team_member]");
		    						$template.find(".team_role").attr("name","teams[N"+current_team_num+"][team_role]");
		    						$template.find(".team_responsibility").attr("name","teams[N"+current_team_num+"][team_responsibility]");
		    						
		    						$template.appendTo("#team_list");
		    						$(".chosen-select").chosen();
		    						current_team_num++;
		    					}
		    					
		    					function show_team(team_id,team_member,team_role,team_responsibility,team_priority)
		    					{
		    						team_priority = typeof team_priority == "undefined"?0:team_priority;
		    						
		    						var $template = $("#team_template").clone();
		    						$template.removeClass("hidden").find("select").addClass("chosen-select");
		    						$template.find(".num").html(current_team_num);
		    						$template.find(".team_id").val(team_id).attr("name","tasks["+team_id+"][team_id]");
		    						$template.find(".team_priority").val(team_priority).attr("name","tasks["+team_id+"][team_priority]");
		    						$template.find(".team_member").val(team_member).attr("name","tasks["+team_id+"][team_member]");
		    						$template.find(".team_role").val(team_role).attr("name","tasks["+team_id+"][team_role]");
		    						$template.find(".team_responsibility").val(team_responsibility).attr("name","tasks["+team_id+"][team_responsibility]");
		    						$template.find(".btn_delete").attr("onclick","delete_team("+team_id+",$(this))");
		    						$template.appendTo("#team_list");
		    						$(".chosen-select").chosen();
		    						current_team_num++
		    					}
		    					
		    					function delete_this_team_member($obj)
		    					{
		    						$obj.parent().parent().remove();
		    					}
		    					
		    					function delete_team(team_id,$obj)
		    					{
		    						$.get("/admin/teams/do_delete/"+team_id);
		    						delete_this_team_member($obj);
		    					}
		    					...
		    					if(teams.length>0)
		    					{
		    						...
		    						{
		    							var team = teams[x];
		    							show_team(team["team_id"],team["member"],team["role"],team["responsibility"],team["priority"]);
		    						}
		    					}
		    				</script>';
		...
		if(isset($_FILES['items']) and isset($_FILES['items']['name']))
		{
			foreach($_FILES['items']['name'] as $item_id =>$item_name)
			{
				...
				if($_FILES['items']['error'][$item_id]==0)
				{
					...
					$newuploadfile = $uploaddir.$fname.'.pdf';
					$convertedJpgPath = $uploaddir.$fname.'.jpg';
					
					if(move_uploaded_file($_FILES['items']['tmp_name'][$item_id],$uploadfile))
					{
						passthru("/opt/openoffice4/program/python /home/DocumentConverter.py ".$uploadfile." ".$newuploadfile);
						passthru("convert -quality 100 -density 300 ".$newuploadfile." ".$convertedJpgPath);
