...
function render_tree(tre)
{
	...
	for(x in tre)
	{
		if(tre[x]==null)
			continue;
		//console.log(tre[x]);
		render_text += "<ul class='droppable'><li class='li_object empty'>&nbsp;-</li></ul> <ul class='droppable'><li id='li_emp_"+tre[x].id+"' class='li_object' parent='"+tre[x].parentid+"'><span class='icon icon-file'></span> <span data-id='"+tre[x].id+"' class='call_chain_item_span'>"+employees[tre[x].id]['First Name']+" "+employees[tre[x].id]['Last Name']+"</span>";
...
function append_item($source,$item)
{
	$item.fadeOut(function(){
		...
		var $li_item = $("<ul class='droppable'><li class='li_object empty'>&nbsp;-</li></ul> <ul class='droppable'><li id='li_emp_"+employee_id+"' class='li_object' parent='"+parent+"'><span class='icon icon-file'></span> <span data-id='"+employee_id+"' class='call_chain_item_span'>"+employees[employee_id]['First Name']+" "+employees[employee_id]['Last Name']+"</span> </li></ul>");
...
{
	var employee_id = $source.attr("id").replace('li_emp_','').replace('emp_','');
	
	var parent=0;
	if($destination.attr('id'))
		parent = $destination.attr('id').replace("li_emp_",'').replace('emp_','');
	
	//console.log("Moving Item");
	//var $clone = $source.proxy();
	
	//var $li_item = $("<li id='li_emp_"+employee_id+"' class='li_object' parent='"+parent+"'><span class='icon icon-file'></span> <span data-id='"+employee_id+"' class='call_chain_item_span'>"+employees[employee_id]['First Name']+" "+employees[employee_id]['Last Name']+"</span></li>");
	...
	if($source.parent().next().find("li.empty").size())
	...
	if($destination.hasClass("empty"))
	{
		...
		console.log("Dropping on empty li");
		var $li_item = $("<li id='li_emp_"+employee_id+"' class='li_object' parent='"+parent+"'><span class='icon icon-file'></span> <span data-id='"+employee_id+"' class='call_chain_item_span'>"+employees[employee_id]['First Name']+" "+employees[employee_id]['Last Name']+"</span> </li>");
		var $ul = $destination.parent();
		$ul.html($li_item);
		$ul.before("<ul class='droppable'><li class='li_object empty'>&nbsp;</li></ul>");
		$ul.after("<ul class='droppable'><li class='li_object empty'>&nbsp;</li></ul>");
		...
		*/
		...
		$source.css("left","0px").css("top","0px").parent().appendTo($destination.parent());
		$source.before("<ul class='droppable'><li class='li_object empty'>&nbsp;</li></ul>");
		$source.after("<ul class='droppable'><li class='li_object empty'>&nbsp;-</li></ul>");
		...
		/*$source.remove();
		var $li_item = $("<ul class='droppable'><li id='li_emp_"+employee_id+"' class='li_object' parent='"+parent+"'><span class='icon icon-file'></span> <span data-id='"+employee_id+"' class='call_chain_item_span'>"+employees[employee_id]['First Name']+" "+employees[employee_id]['Last Name']+"</span> </li></ul>");
		$destination.append($li_item);
		apply_droppable($li_item.find("li"));*/
		//var $copy = $source.parent().clone(false);
		//$source.parent().remove();
		$source.css("left","0px").css("top","0px").parent().appendTo($destination);
		
		$source.parent().before("<ul class='droppable'><li class='li_object empty'>&nbsp;-</li></ul>");
		$source.parent().after("<ul class='droppable'><li class='li_object empty'>&nbsp;-</li></ul>");
	...
	/*$("#droppable").find("li.empty").each(function(){
		if($(this).next().find("li.empty").size())
			$(this).next().remove();
	});*/
	
	
	
	//$destination.find(".droppable:last-of-type").append($li_item);
	//});
}
...
{
	var items_to_save=[];
	$("#droppable li").each(function(index,itm){
