...
function render_tree(tre)
{
	...
	for(x in tre)
	{
		if(tre[x]==null)
			continue;
		//console.log(tre[x]);
		render_text += "<ul id='ul_"+tre[x].id+"' class='droppable'><li id='li_emp_"+tre[x].id+"' parent='"+tre[x].parentid+"'><span class='icon icon-file'></span> <span data-id='"+tre[x].id+"' class='call_chain_item_span'>"+employees[tre[x].id]['First Name']+" "+employees[tre[x].id]['Last Name']+"</span>";
...
//	});
  ...
  var $table = $("#table");
   ...
   // $( "#draggable" ).draggable();
	setTimeout(apply_droppable($('.droppable li')), 200);
...
function append_item($source,$item)
{
	$item.fadeOut(function(){
		...
		var $li_item = $("<ul id='ul_"+employee_id+"' class='droppable'><li id='li_emp_"+employee_id+"' parent='"+parent+"'><span class='icon icon-file'></span> <span data-id='"+employee_id+"' class='call_chain_item_span'>"+employees[employee_id]['First Name']+" "+employees[employee_id]['Last Name']+"</span> <ul class='droppable'><li class='empty'></li></ul>  </li></ul>");
		
		apply_droppable($li_item.find("li"));
		/*$li_item.find("li").droppable({accept: ".employee",hoverClass: "li_hover", greedy: true,drop: function( event, ui ) {
			append_item($(this),ui.draggable); 
		} });
		
		$li_item.find("li").draggable({
		   		revert:"invalid",
		   		containment:"document",
		   		helper:"clone", //original
		   		cursor:"move"
