...
var current_folder = 'super_admin';
</script>
    ...
    <!-- Sidebar ends -->
      ...
      <div class="page-head">
        ...
        <div class="bread-crumb pull-left">
			...
			<span class="divider">/</span> 
			<a href="/super_admin/library_items/add/" class="bread-current"> &raquo; Add New Field</a>
          </div>
	          	              ...
	          	              <div class="widget wgreen">
	          	                ...
	          	                <div class="widget-content">
	          	                  <div class="padd">
	          	                    ...
	          	                    <!-- Form starts.  -->
	          	                     <form class='form-horizontal' name='frm_add_library_items' method='post' action='/super_admin/library_items/do_add/' onSubmit='return form_validation(this);' enctype='multipart/form-data'>
	          	                     <table class="table table-bordered table-striped table-hover content" style="width:100%;" cellpadding="0" cellspacing="0">
	          	                     <input type='hidden' id='chaabee_post' name='chaabee_post' value=''><input type='hidden' id='id' name='id' value=''>
	          	                     <input type="hidden" name="library_id" value="<?=$library_details['id']?>" />
	          	                     <? /*
	          	                     <tr>
										<td width='35%' class='capitalize' valign='top' align='left'>library :</td>
										<td width='65%' class='capitalize'><select class='form-control' name='library_id' id='library_id'><? 
										
										$res = $this->db->query("select * from `library`");
										if($res->num_rows()>0)
										{
										...
										*/ 
										?>
											<tr>
												...
												<td width='65%' class='capitalize'><input  class='form-control ' type='text' size=40 name='var_name'  value=''  /></td>
											</tr>
											<tr>
											...
											<tr class="dropdown hidden">
												...
												<td>
													<select name="dropdown_id" id="dropdown_id">
														<?
														$res = $this->db->query("select dropdown_id,`name` from `dropdown` where company_id=0");
														if($res->num_rows()>0)
														{
															foreach ($res->result_array() as $data)
														 	{
											...
											<tr class="library_dropdown hidden">
												<td valign='top' align='left'>Lbirary:</td>
												<td>
													<div class="pull-left">
													<select name="library_dropdown_id" id="library_dropdown_id" onchange="library_selected(this.value)">
														...
														<?
														$res = $this->db->query("select id,`name` from `library` where company_id=0");
														if($res->num_rows()>0)
														{
															foreach ($res->result_array() as $data)
														 	{
											...
											<td width='65%' class='capitalize'><input class='form-control' type='text' name='item_order' id='item_order'  value='0' /></td>
											</tr>
