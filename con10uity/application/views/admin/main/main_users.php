...
function do_search(search) {
	$.post("/admin/users/main/?output=ajax&search="+search,{},function(ret){
		var test = ret;
						...
						<div class="widget">
					        <div class="widget-head">
			                  ...
			                  <div class="pull-left" style="margin-left: 20px;">
			                   	<? if($CI->permission_lib->has_permission("user-setup","add")){ ?>
			                    	<a href="/admin/users/add/" class="btn btn-mini btn-primary">Add</a>
			                    <? } ?>
			                   </div>
								...
								<td class="rowHeader"><?                 
			                	    ...
			                	    <?                
			                	        ...
			                	        ?>
			                	            ...
			                	            <td><a class="btn btn-danger" target="_blank" href="/auth/login/?username=<?=urlencode($row['username'])?>&password=<?=urlencode($row['password'])?>&company=<?=urlencode($row['fcompany_id'])?>">Impersonate As</a></td>
			                	            
			                	            	<td align="center" valign="top" class="rowDark">
			                	            		<? if($CI->permission_lib->has_permission("user-setup","edit")){ ?>
			                	            			<a style="text-decoration:none" class="btn btn-xs btn-warning" href="/admin/users/edit/<?=$row['user_id']?>/"><i class="icon-pencil"></i> </a>
			                	            		<? } ?>
			                	            		<? if($CI->permission_lib->has_permission("user-setup","delete")){ ?>
