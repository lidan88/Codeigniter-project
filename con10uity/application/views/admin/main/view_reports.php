...
function delete_items()
{
	...
	if(p)
	{
		...
		{
			$.post("/admin/reports/do_delete_multi/",{"ids":searchIDs.join(",")},function(ret){
...
function edit_item()
{
	...
	if(searchIDs.length>0)
	{
		...
		{
			top.location="/admin/reports/edit/"+searchIDs[0];
		}
		else {
...
function do_search(search) {
	$.post("/admin/reports/view/?output=ajax&search="+search,{},function(ret){
		var test = ret;
...
function viewFields(report_id)
{

	$.post("/admin/reports/view_fields/"+report_id,{},function(ret){
			
			if(ret!='undefined' && ret!='null')
			{
				var itemsToReturn = processtemplate(ret,[]);
				$("#modalViewfields").find(".modal-body").html(itemsToReturn.join(' , '));
				$("#modalViewfields").modal('show');
			}
	},'json');
}

function processtemplate(items,itemsToReturn)
{
	//var itemsToReturn = [];
	for(x in items)
	{
		//console.log(ret[x]);
		itemsToReturn.push('<strong>'+items[x]['title']+'</strong>');
		if(typeof items[x]['children']!='undefined')
		{
			itemsToReturn = processtemplate(items[x]['children'],itemsToReturn);
		}
	}
...
$(document).ready(function(){
	$( "tr" ).dblclick(function() {
	  ...
	  top.location="/admin/reports/step/2/"+item_no+"/?edit=1";
    ...
    <!-- Sidebar ends -->
  	...
  	<div class="mainbar">
		...
		<div class="bread-crumb pull-left">
		  ...
		  <span class="divider">/</span> 
		  <a href="/admin/reports/add/"> &raquo; Add New</a>
		</div>
						...
						<div class="widget">
					        <div class="widget-head">
			                  ...
			                  <div class="pull-left" style="margin-left: 20px;">
			                  	...
			                  	<a href="/admin/reports/step/1/" class="btn btn-mini btn-primary">Add</a>
			                	    ...
			                	    <?                
			                	        ...
			                	        ?>
			                	            <tr id="item_no_<?=$row['id']?>">
												...
												<td align="left" valign="top"><a target="_blank" href="/admin/reports/step/2/<?=$row['id']?>/?edit=1">Print It</a></td>
												<td align="left" valign="top"><a target="_blank" href="javascript:viewFields(<?=$row['id']?>);">View Fields</a></td>
...
<div id="modalViewfields" class="modal fade">
